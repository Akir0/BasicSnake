...
public class BodySnakeFollowPlayer : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_target != null)
        {
            ...
            _characterController.Move(moveDirection.normalized * _speed * Time.deltaTime); // Je dis à l'ennemi de bouger avec la direction FOIS la vitesse FOIS deltaTime pour calquer sur le temps du jeu
        }
    }
}